...
public class Fast2 {
	...
	public static boolean alreadyFound ( List<Point> lines, List<Point> lis ) {
		
		for(int i = 0; i < lines.size() - 1; i++) {
			
			if( lines.get( i ) == lis.get(0) &&
				lines.get( i + 1 ) == lis.get(1)) { return true; }
		} return false;
	}
	...
	public static void main(String[] args) {
		...
		List<Point> collinearList = new ArrayList<Point>( );
			...
			for(int j = i; j < n - 1; j++) {
				...
				// Array is printed, then cleared and everything re-initiated
					...
					if( num_of_equals >= 3) {
						...
						if( !alreadyFound( collinearList, tmpLis ) ) {
							
							collinearList.addAll( tmpLis );
							printList ( tmpLis );
						}
					}
